  ...
  // Rating Star 12057:12172 (iYqedQS2TuLL3JBkbgw7ak) — the mobile card's rating row
  function svgStar() {
    return `<svg width="12" height="12" viewBox="0 0 11.9997 12" fill="none"><path d="M2.23028 11.9996C2.08002 11.9996 1.93042 11.9488 1.80916 11.848C1.58443 11.6622 1.5093 11.3498 1.62463 11.0816L2.9666 7.49983L0.270987 4.96248C0.0403272 4.79443 -0.0558909 4.49721 0.0324189 4.22503C0.120729 3.95417 0.373795 3.7703 0.659155 3.7703H4.26799L5.3745 0.450775C5.46413 0.181892 5.71588 0 5.99992 0C6.2833 0 6.53505 0.181892 6.62534 0.450775L7.73118 3.7703H11.3407C11.626 3.7703 11.8791 3.95417 11.9674 4.22503C12.0557 4.49721 11.9588 4.79443 11.7282 4.96248L9.00647 7.51983L10.076 11.1191C10.1735 11.3913 10.0819 11.6958 9.84996 11.8691C9.61798 12.0424 9.30033 12.0437 9.06638 11.8724L5.98344 9.61522L2.59736 11.8875C2.48598 11.9633 2.35747 11.9996 2.23028 11.9996Z" fill="#FCB52B"/></svg>`;
  }
  ...
  // Icons 18 / Search (2144:13058) — the Brands sheet's search box
  function svgSearch() {
    return `<svg width="22" height="22" viewBox="0 0 22 22" fill="none"><path d="M14.793 15.293C15.184 14.902 15.816 14.902 16.207 15.293L19.207 18.293C19.598 18.684 19.598 19.316 19.207 19.707C19.012 19.902 18.756 20 18.5 20C18.244 20 17.988 19.902 17.793 19.707L14.793 16.707C14.402 16.316 14.402 15.684 14.793 15.293ZM9.5 2C13.36 2 16.5 5.141 16.5 9C16.5 12.859 13.36 16 9.5 16C5.64 16 2.5 12.859 2.5 9C2.5 5.141 5.64 2 9.5 2ZM9.5 4C6.743 4 4.5 6.243 4.5 9C4.5 11.757 6.743 14 9.5 14C12.257 14 14.5 11.757 14.5 9C14.5 6.243 12.257 4 9.5 4Z" fill="#757575"/></svg>`;
  }
  ...
  function buildFilterContent(container, pillId) {
    ...
    // Search box 2185:29440 — the Brands sheet filters its own list. Sheet only:
    // the web popover has no search field.
    const searchable = pillId === 'brands' && container === sheetBody;
    if (searchable) container.appendChild(brandSearchBox(container));
    buildValueList(container, pillId, attrValues(pillId));
    if (searchable) applyBrandQuery(container);
    syncClearBtn(pillId);
  }

  // Ticking a row rebuilds the list, so the query outlives the rebuild here
  // rather than in the input, and rows are hidden rather than dropped.
  let brandQuery = '';

  function brandSearchBox(list) {
    const box = document.createElement('div');
    box.className = 'sheet-search';
    box.innerHTML = `<span class="sheet-search-icon">${svgSearch()}</span>`;
    const input = document.createElement('input');
    input.type = 'search';
    input.className = 'sheet-search-input';
    input.placeholder = 'Search brands';
    input.value = brandQuery;
    input.addEventListener('input', () => {
      brandQuery = input.value;
      applyBrandQuery(list);
    });
    box.appendChild(input);
    return box;
  }

  function applyBrandQuery(list) {
    const q = brandQuery.trim().toLowerCase();
    list.querySelectorAll('.val-row').forEach(row => {
      const label = row.querySelector('.val-label');
      row.hidden = !!q && label.textContent.toLowerCase().indexOf(q) === -1;
    });
  }
  ...
  // Escape backs out of whichever surface is up, discarding the draft either way
  ...
  function openSheet(pillId) {
    ...
    if (pillId === 'category') seedCategoryDraft();
    beginDraft(pillId);
    sheetTitle.textContent = pillId === 'sort'
      ? 'Sort by'
      : (resolvedPills().find(p => p.id === pillId) || { label: '' }).label;
  ...
  // Each sheet names its own buttons, and the three pairs come straight off the
  // frames: the attribute sheets (156:20715) say Clear / Apply, Sort by and
  // Category (2144:14221 / 2144:20952) say Reset / Done — they resolve to a single
  ...
  // as rows, which is what makes them the attribute sheets.
  function isAttrGridPill(pillId) {
    return pillId === 'color' || SIZE_FAMILY_KEYS.indexOf(pillId) !== -1;
  }

  function updateSheetCta() {
    if (!sheet.classList.contains('open')) return;
    const labels = sheetLabels(openPillId);
    sheetClear.textContent = labels.clear;
    sheetCta.textContent   = labels.cta;
  }
  ...
  sheetCta.addEventListener('click', () => closeSheet(true));
  ...
  // ── Drag to scroll ────────────────────────────────────────────
  // Both filter rows overflow and hide their scrollbars, so a mouse has no way to
  // reach the pills past the right edge — touch already pans them natively, and
  // the arrows only exist on web. Mouse drag covers the gap. A drag that actually
  // moved swallows the click it would otherwise land on a pill.
  const DRAG_SLOP = 4;
  ...
  function enableDragScroll(el) {
    ...
    el.addEventListener('pointerdown', e => {
      ...
      // the row scrolls smoothly for the arrow buttons; a drag has to track the
      // pointer, so the animation is off for as long as the drag lasts
  ...
  enableDragScroll(document.getElementById('pillsRow'));
  enableDragScroll(document.getElementById('appliedRow'));

  // ── Stuck state ───────────────────────────────────────────────
  // The bar carries its rule only once it has actually stuck — at rest it sits
  // flush against the content above it and draws nothing. `top` resolves to 0 on
  // web and to the nav height on mobile, so comparing against the computed value
  // covers both without the breakpoint being named here.
  function syncStuck() {
    const bar = document.querySelector('.filter-section');
    const stickTop = parseFloat(getComputedStyle(bar).top) || 0;
    bar.classList.toggle('is-stuck', bar.getBoundingClientRect().top <= stickTop + 0.5);
  }
  ...
  function sortLabel() {
    const opt = SORT_OPTIONS.find(o => o.id === sortSelected);
    return opt ? opt.label : 'Relevance';
  }

  function pillLabel(p) {
    // 1450:8379 spells the mobile pill out in full; the web row (4:1549) doesn't
    if (p.id === 'all-filters') return isMobile() ? 'All Filters' : p.label;
    // Web keeps the chosen sort inside the popover; mobile names it on the pill
    // ("By Relevance", 1450:8506), because the sheet covers the row that opened it
    if (p.id === 'sort') return isMobile() ? `By ${sortLabel()}` : p.label;
  ...
  function renderPills() {
    ...
    resolvedPills().forEach(p => {
      ...
      if (p.icon === 'filter') btn.innerHTML += `<span class="pill-icon pill-icon--filter">${svgFilter(iconColor)}</span>`;
      else if (p.icon === 'sort') btn.innerHTML += `<span class="pill-icon pill-icon--sort">${svgSort(iconColor)}</span>`;
  ...
  function renderApplied() {
    ...
    if (chips.length) {
      ...
      clearBtn.textContent = isMobile() ? 'Clear All' : 'Clear all';
  ...
  function renderBrandRow() {
    ...
    // 1450:8454 — the mobile link sits under the strip and carries the count, so
    // it can't ride inside the row: the row scrolls and would carry it off screen.
    const seeAllHost = document.getElementById('brandSeeAll');
    seeAllHost.innerHTML = '';
    const all = document.createElement('button');
    all.className = 'brand-all';
    const label = isMobile() ? `See all (${availableCount('brands')})` : 'See all';
    all.innerHTML = `<span>${label}</span><span class="cat-chev-r">${svgChevSmRight()}</span>`;
    all.addEventListener('click', e => { e.stopPropagation(); openAllFilters(); });
    (isMobile() ? seeAllHost : row).appendChild(all);
  }
  ...
  // The mobile card (1389:7193) carries two promo pills and a rating row. The
  // fixture has neither field, so both read off the same deterministic sources the
  // sort stand-ins use: discount depth for Flash Deals, the title hash for the
  // rest. Figma draws five filled stars on every card, so STAR_COUNT is fixed.
  const STAR_COUNT = 5;
  const FLASH_DEAL_MIN_SAVE = 50;
  function isFlashDeal(p)   { return saveOf(p) >= FLASH_DEAL_MIN_SAVE; }
  function isNewArrival(p)  { return pseudoRating(p) % 3 === 0; }
  function expertCount(p)   { return 5 + (pseudoRating(p) % 95); }
  ...
  function renderProducts() {
    ...
    list.forEach(p => {
      ...
      const stars = svgStar().repeat(STAR_COUNT);
      card.innerHTML = `
        <div class="pli-badges">
          ${isFlashDeal(p) ? '<span class="pli-badge pli-badge--flash">Flash Deals</span>' : ''}
          ${isNewArrival(p) ? '<span class="pli-badge pli-badge--new">New</span>' : ''}
        </div>
        ...
        <div class="pli-rating">
          ...
          <span class="pli-reviews">${expertCount(p)} experts</span>
        </div>
        <button class="pli-cta">Add to Cart</button>
      `;
  ...
  // 1423:28286 labels the sheet's chrome differently from the web flyout: the
  // panel is the whole surface on mobile, so the buttons name the outcome.
  function syncAFChrome() {
  ...
  MOBILE_Q.addEventListener('change', () => {
    ...
    // labels ("Filters" / "All Filters") and the See all host both switch here
    syncAFChrome();
    render();
  });
